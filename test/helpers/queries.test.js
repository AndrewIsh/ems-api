...
jest.mock('../../../ems-db', () => {
    return {
        resolvers: {
            queries: {
                initiators: jest.fn().mockImplementation(() => new Promise(
                    (resolve) => resolve(
                        {
                            rows: [
                                { id: 1, initiator: 1 },
                ...
                participants: jest.fn().mockImplementation(() => new Promise(
                    (resolve) => resolve(
                        {
                            rows: [
                ...
                labels: jest.fn().mockImplementation(() => new Promise(
                    (resolve) => resolve(
                        {
                            rows: [
                ...
                latestMessages: jest.fn().mockImplementation(() => new Promise(
                    (resolve) => resolve(
                        {
                            rows: [
...
describe('queries', () => {
    describe('addEmbeds', () => {
        ...
        it('should call initiators with the IDs of the passed queries', (done) => {
            expect(db.resolvers.queries.initiators).toHaveBeenCalledWith([ 1, 2 ]);
            done();
        });
        it('should call participants with the IDs of the passed queries', (done) => {
            expect(db.resolvers.queries.participants).toHaveBeenCalledWith([ 1, 2 ]);
            done();
        });
        ...
        it('should call latest with the IDs of the passed queries', (done) => {
            expect(db.resolvers.queries.latestMessages).toHaveBeenCalledWith([ 1, 2 ]);
            done();
        });
