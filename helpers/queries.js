...
const queries = {
    addEmbeds: async (queries) => {
        ...
        const labels = await db.resolvers.queries.labels(query_ids);
        // Finally get the most recent message for each retrieved query
        const latest = await db.resolvers.queries.latestMessages(query_ids);
        ...
        return queries.map((query) => {
            ...
            const queryParticipants = participants.rows
                .filter((participant) => participant.query_id === query.id)
                .map((final) => final.creator_id);
            const queryLabels = labels.rows
                .filter((label) => label.query_id === query.id)
